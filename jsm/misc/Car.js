...
var Car = ( function ( ) {
	...
	var RR_height, RL_height, FR_height, FL_height;
	...
	class Car {
		constructor() {
			...
			this.stiffnessRear = 50000;
			this.stiffnessFront = 50000;
			...
			this.mpg = 5; // Fuel consumption

			// Terrain
			this.bumpiness = 0.2;
			this.bumpHeight = 1.5; // Metres
		...
		getInputs(dt) {
			...
			// Brake position
			if (map[40] || map[83]) {
				this.brake = Math.min(1, this.brake + dt * 5);
				//this.throttle = 0;
			} else {
				this.brake = Math.max(0, this.brake - dt * 5);
			}
		...
		computeWeightTransfer() { // wt stands for Weight Transfer
			...
			this.CM_Pos = this.root.children[0].getWorldPosition(new THREE.Vector3());
			...
			this.wtLateralRear = ((this.localAcceleration.y / this.gravity) * this.axleWeightRear * this.height / this.trackWidth) * this.weightTransfer;
			this.wtLateralFront = ((this.localAcceleration.y / this.gravity) * this.axleWeightFront * this.height / this.trackWidth) * this.weightTransfer;
			...
			this.wtRoll = (Math.sign(this.localAcceleration.y) * this.height * Math.sin(Math.abs(this.bodyRoll)) / this.trackWidth) * weight * this.weightTransfer;
			...
			/*RR_height = (noise.simplex2(car.root.children[1].getWorldPosition(new THREE.Vector3(0, 0, 0)).x*this.bumpiness, car.root.children[1].getWorldPosition(new THREE.Vector3(0, 0, 0)).z*this.bumpiness) + 1)*this.bumpHeight;
			RL_height = (noise.simplex2(car.root.children[2].getWorldPosition(new THREE.Vector3(0, 0, 0)).x*this.bumpiness, car.root.children[2].getWorldPosition(new THREE.Vector3(0, 0, 0)).z*this.bumpiness) + 1)*this.bumpHeight;
			FL_height = (noise.simplex2(car.root.children[3].getWorldPosition(new THREE.Vector3(0, 0, 0)).x*this.bumpiness, car.root.children[3].getWorldPosition(new THREE.Vector3(0, 0, 0)).z*this.bumpiness) + 1)*this.bumpHeight;
			FR_height = (noise.simplex2(car.root.children[4].getWorldPosition(new THREE.Vector3(0, 0, 0)).x*this.bumpiness, car.root.children[4].getWorldPosition(new THREE.Vector3(0, 0, 0)).z*this.bumpiness) + 1)*this.bumpHeight;

			let slope = 0.5;
			RR_height = Math.max(0, -car.root.children[1].getWorldPosition(new THREE.Vector3(0, 0, 0)).z*slope-1);
			RL_height = Math.max(0, -car.root.children[2].getWorldPosition(new THREE.Vector3(0, 0, 0)).z*slope-1);
			FL_height = Math.max(0, -car.root.children[3].getWorldPosition(new THREE.Vector3(0, 0, 0)).z*slope-1);
			FR_height = Math.max(0, -car.root.children[4].getWorldPosition(new THREE.Vector3(0, 0, 0)).z*slope-1);
			...
			*/
			...
			this.RR_wheelPos.y = RR_height;
			this.RL_wheelPos.y = RL_height;
			this.FL_wheelPos.y = FL_height;
			this.FR_wheelPos.y = FR_height;
			...
			this.dx = Math.max(maxRight-maxLeft, minLeft-maxRight);
			this.dz = Math.max(maxRear-minFront, minRear-maxFront);

			this.angleX = Math.asin(this.dz/this.wheelBase);
			this.angleZ = Math.asin(this.dx/this.trackWidth);

			// Compute weight transfer in lateral and longitudinal direction
			this.wtX = (this.height * Math.sin(this.angleZ) / this.wheelBase) * weight;
			this.wtZ = (this.height * Math.sin(this.angleX) / this.trackWidth) * weight;

			// Update each wheel weight
			this.axleWeightFront += this.wtZ;
			this.axleWeightRear -= this.wtZ;

			this.rearR += this.wtX/2 + this.wtZ/2;
			this.rearL -= this.wtX/2 + this.wtZ/2;
			this.frontR += this.wtX/2 + this.wtZ/2;
			this.frontL -= this.wtX/2 + this.wtZ/2;

			

			//console.log(this.wtLateral, this.wtRoll, this.wtX, this.wtZ) // Log the lateral weights
			...
			RL_FR2.sub(this.RL_bodyRel).normalize().divideScalar(1);
			RL_FR2.y += 0.248;
			let RR_FL2 = this.FL_bodyRel.clone();
			RR_FL2.sub(this.RR_bodyRel).normalize().divideScalar(1);
			RR_FL2.y += 0.248;
			let RL_FL2 = this.FL_bodyRel.clone();
			RL_FL2.sub(this.RL_bodyRel).normalize().divideScalar(1);
			RL_FL2.y += 0.289;
			let RL_RR2 = this.RR_bodyRel.clone();
			RL_RR2.sub(this.RL_bodyRel).normalize();
			let RR_FR2 = this.FR_bodyRel.clone();
			RR_FR2.sub(this.RR_bodyRel).normalize().divideScalar(1);
			RR_FR2.y += 0.289;

			/*if (Math.random() > 0.9)
				console.log(RL_FR, RL_FR2, RR_FL, RR_FL2, RL_FL, RL_FL2, RL_RR, RL_RR2, RR_FR, RR_FR2)*/

			this.root.children[0].setRotationFromQuaternion(new THREE.Quaternion(-0.5, -0.5, -0.5, 0.5));
			...
			this.root.children[0].applyQuaternion(q1);
			this.root.children[0].applyQuaternion(q2);
			this.root.children[0].applyQuaternion(q3);
			this.root.children[0].applyQuaternion(q4);
			this.root.children[0].applyQuaternion(q5);

			this.root.children[0].rotation.x -= this.angleX;
			this.root.children[0].rotation.z -= this.angleZ;
		}

		calculateCornerFriction() {
			...
			this.frictionForceFront = Math.clamp(-this.cornerStiffnessFront * this.slipAngleFront, -tireGripFront, tireGripFront) * this.axleWeightFront;
			this.frictionForceRear = Math.clamp(-this.cornerStiffnessRear * this.slipAngleRear, -tireGripRear, tireGripRear) * this.axleWeightRear;
		}
		...
		applyForces(dt) {
			...
			this.corneringForce = Math.cos(this.steeringAngle) * this.frictionForceFront + this.frictionForceRear;
			...
			let angularTorque = (this.frictionForceFront + tractionForce.y) * this.cgToFrontAxle - this.frictionForceRear * this.cgToRearAxle;
			...
			this.velocityDelta = this.velocity.copy();
			this.velocityDelta.mult(dt*length);
		...
		updateCar(dt) {
			// Set car position
			this.root.position.x -= this.velocityDelta.y;
			this.root.position.z -= this.velocityDelta.x;

			// Set car angle
			this.root.rotation.y = this.carAngle;

			// Car height
			this.terrainHeight = (RR_height + RL_height + FL_height + FR_height)/4;
			this.root.children[0].position.y = this.height + this.terrainHeight;
			...
			steeringWheel.position.y = 0.8 + this.terrainHeight;
		}

		calculateFuel(dt) {
			// Calculate fuel
			this.fuel = 18 - 1/this.mpg*toMiles(this.distanceTraveled);
			if (this.fuel < 0) {
				this.fuel = 0;
				this.throttle = 0;
			}
		...
		applyPhysics(dt) {
			...
			this.computeWeightTransfer(dt);

			this.calculateCornerFriction(dt);
